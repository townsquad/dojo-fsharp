...
[<Fact>]
let ``Empty strands`` () = distance "" "" |> should equal (Some 0)
...
[<Fact>]
let ``Disallow left empty strand`` () = distance "" "G" |> should equal None

[<Fact>]
let ``Disallow right empty strand`` () = distance "G" "" |> should equal None
